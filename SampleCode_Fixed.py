...
💡 **Want to see more data or download the full dataset? Just ask!**
"""
        return response
    ...
    async def process_user_input(self, user_input: str) -> str:
        ...
        try:
            ...
            if intent == "query_adx":
                # For demo purposes, return sample ADX data
                result = self.generate_sample_data(DataSource.ADX)
                self.last_query_result = result
                return self.format_result_for_user(result)
            
            elif intent == "query_sharepoint":
                # For demo purposes, return sample SharePoint data
                result = self.generate_sample_data(DataSource.SHAREPOINT)
                self.last_query_result = result
                return self.format_result_for_user(result)
            
            elif intent == "download_full_data":
                if self.last_query_result:
                    return f"📥 **Full Data Download**\n\nPreparing download for {self.last_query_result.total_rows} rows from {self.last_query_result.source.value.upper()}...\n\n(In production, this would trigger the actual data download)"
                else:
                    return "❌ No previous query to download. Please run a query first."
            
            elif intent == "show_sample":
                if self.last_query_result:
                    return self.format_result_for_user(self.last_query_result)
                else:
                    return "❌ No previous query to show. Please run a query first."
            
            else:
                return "🤖 **SembSolar Data Assistant**\n\nI can help you with:\n• Querying Azure Data Explorer (ADX)\n• Accessing SharePoint documents\n• Downloading sample or full datasets\n\nWhat would you like to explore today?"
...
async def main():
    ...
    print("🌞 SembSolar Data Intelligence Assistant Started!")
